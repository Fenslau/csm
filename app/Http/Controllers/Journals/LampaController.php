...
class LampaController extends Controller
{
    ...
    public function new(LampaRequest $request) {
        ...
        if ($duration_all/60 > env('LAMPA_NARABOsKA')) session()->flash('warning', 'Пора заменить лампу в помещении <b>'.$request->lampa.'</b> Время наработки: <b>'.round($duration_all/60, 0) .' '. trans_choice('час|часа|часов', round($duration_all/60, 0), [], 'ru').'</b>');
